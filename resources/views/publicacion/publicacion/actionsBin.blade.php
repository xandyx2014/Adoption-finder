<a class="btn btn-success" href="{{ route('publicacion.show', $id) }}">
    <i class="fa fa-eye" aria-hidden="true"></i>
</a>
<form method="POST" action="{{ route('publicacion.update', $id) }}?restore=true" style="display: inline">
    @csrf
    @method('PUT')
        ...
        $("#especie-delete-{{$id}}").click(async function (e) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const {data} = await axios.delete("{{ route('publicacion.destroy', $id) }}?bin=true");
