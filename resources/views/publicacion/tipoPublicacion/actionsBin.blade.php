...
@can('permiso', 'estado-tipo-publicacion')
    <form method="POST" action="{{ route('tipopublicacion.update', $id) }}?restore=true" style="display: inline">
        @csrf
        @method('PUT')
...
@can('permiso', 'eliminar-tipo-publicacion')
        ...
        $("#especie-delete-{{$id}}").click(async function (e) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const {data} = await axios.delete("{{ route('tipopublicacion.destroy', $id) }}?bin=true");
