...
@section('content')
    ...
    <div class="container elevation-4">
        <form action="{{ route('publicacionAdopcion.update', $publicacion->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-9 p-2">
                    <div class="mb-3">
                        ...
                        <input name="titulo" value="{{ old('titulo', $publicacion->titulo) }}" type="text" class="form-control @error('titulo') is-invalid @enderror"  placeholder="Mi titulo de mi publicacion">
                        @error('titulo')
                    ...
                    <ul class="list-group mb-3">
                        <li class="list-group-item d-flex justify-content-between lh-condensed">
                            <div>
                                ...
                                <select name="mascota" value="{{ old('mascota', $publicacion->mascota_id) }}" class="js-example-basic-single w-100">
                                    @foreach($mascotas as $mascota)

                                        <option
                                            @if($publicacion->mascota_id == $mascota->id)
                                                selected
                                            @endif
    ...
    @push('js')
        ...
        <script type="text/javascript">
            ...
            sum.summernote('code', `{!! old('descripcion_corta', $publicacion->descripcion_corta) !!}`);
        </script>
    @endpush
@endsection
