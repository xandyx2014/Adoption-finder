@can('permiso', 'consultar-rol')
    <a class="btn btn-success elevation-2" href="{{ route('rol.show', $data->id) }}">
        <i class="fa fa-eye" aria-hidden="true"></i>
    </a>
@endcan
@can('permiso', 'estado-rol')
    <form method="POST" action="{{ route('rol.update', $data->id) }}?restore=true" style="display: inline">
        @csrf
        @method('PUT')
            ...
            $("#especie-delete-{{$data->id}}").click(async function (e) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        const {data} = await axios.delete("{{ route('rol.destroy', $data->id) }}?bin=true");
